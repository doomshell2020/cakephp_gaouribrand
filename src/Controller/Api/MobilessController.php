...
class MobilessController extends AppController
{
  ...
  {
    parent::initialize();
    $this->Auth->allow(['registration', 'loginOtpVerify', 'prepareRequestBody', 'registrationOtpVerify', 'login', 'resendOtp', 'contactInformation', 'home', 'faqCategories', 'faqQuestion', 'couponsList', 'uploadToken', 'checkreferralcode', 'enterReferralCoderegistration', 'contactUs', 'privacyPolicy', 'termsAndConditions', 'getLocations']);
  }
  ...
  public function enterReferralCoderegistration($referal_code_data = null, $userId)
  {
    ...
    if ($this->request->is('post')) {
      ...
      // $user_data = $this->Users->find('all')->where(['id' => $userId,'referred_user_id is not NULL'])->count();
      // if ($user_data > 0) {
      // 	$response['success'] = false;
      // 	$response['message'] = "You have already updated referral code";
      // 	$this->response->type('application/json');
      // 	$this->response->body(json_encode($response));
      // 	return $this->response;
      // }
  ...
  public function viewProfile()
  {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
      ...
      if (count($result) > 0) {
        ...
        $product["url"] = "https://demo.gaouribrand.com/referral_info/$id";
        //$product["email"]=$result['email'];
        $qr_code = $this->QRCodeGenerator($product["user_id"], $product["url"]);
        $product["qrcode_image"] = "/var/www/html/webroot/QRImages/temp/$qr_code";
  ...
  public function viewCart()
  {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      if (count($result) > 0) {
        ...
        // pr($product['vendor_id']);exit;
        if ($coupons) {
          if ($coupons['vendor_id'] != $product['vendor_id']) {
            $response["success"] = false;
          ...
          if (date('Y-m-d', strtotime($coupons['valid_to'])) >= $today_date) {
            if ($coupons['applicable_type'] == "amount") {
              ...
              if ($discount > $coupons['maximum_discount']) {
                ...
                $discount = $total * $promo_code['discount_rate'] / 100;
              }
              $promoCodeMessage = "कूपन कोड सफलतापूर्वक लागू किया गया";
            }
          } else {
  ...
  public function userAddresses()
  {
    ...
    if ($this->request->is('post')) {
      ...
      $service_area = $this->Servicearea->find()->toarray();
      $serviceArea = [];
  ...
  public function orderCart()
  {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      if (count($result) > 0) {
        ...
        $locality = $this->Comman->get_location_name($res['location_id']);
        ...
        if ($coupons) {
          ...
          if ($coupons['vendor_id'] != $location_data['vendor_id']) {
            $response["success"] = false;
            $response["message"] = "आपका कूपन इस विक्रेता के प्रोडक्ट पर लागू नहीं है";
            $this->response->type('application/json');
            $this->response->body(json_encode($response));
  ...
  public function getLocations()
  {
    ...
    $locations = $this->Locations->find('all')->toarray();
    ...
    $data = [];
    $locations = $this->Servicearea->find('all')->where(['vendor_id IS NOT NULL'])->group(['vendor_id'])->toarray();
    if (empty($matchedLocationID)) {
      $matchedLocationID = 1;
    }
    ...
    if (empty($locations)) {
      $response = array();
      $response['success'] = false;
      $response['message'] = "कोई स्थान नहीं मिला";
      echo json_encode($response);
      return;
    }
    $response['success'] = true;
    $response['selectedId'] = $matchedLocationID;
    foreach ($locations as $locations) {
      $vendorbase_location = $this->Locations->find('all')->where(['id' => $locations['location_id']])->first();
      $data['location_id'] = $vendorbase_location['id'];
      $data['name'] = $vendorbase_location['name'];
